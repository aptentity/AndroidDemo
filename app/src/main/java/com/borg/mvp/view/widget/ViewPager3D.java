	...
	protected boolean getChildStaticTransformation(View child, Transformation t) {
		...
		if (mOverscrollEffect.isOverscrolling() && isFirstOrLast) {
			...
			if (mAnimateAlpha) {
				...
				t.setAlpha((float)(Math.sin((float) ((1 - Math.abs(mOverscrollEffect.mOverscroll)) * Math.PI / 2))));
			}
			return true;
			...
			if (mTempTect.left >= mScrollPositionOffsetPixels) {
				if (mAnimateAlpha) {
					t.setTransformationType(Transformation.TYPE_BOTH);
					t.setAlpha((float)(Math.sin((float) (mScrollPositionOffset
							* Math.PI / 2))));
				}
				// right side
			...
			final float translateZ = (mSwipeTranslation * (float)(Math.sin((float) ((Math.PI) * mScrollPositionOffset))));
